...
function Spawn( entityKeyValues )
	...
	thisEntity.DBlast = thisEntity:FindAbilityByName("greevil_white_deafening_blast")
...
function AggressiveThink()
	...
	if #units > 0 then
		if thisEntity.DBlast:IsFullyCastable() then
			CastDBlast()
			return true
		end
	end
...
function CastDBlast()
	...
	thisEntity:CastAbilityNoTarget(thisEntity.DBlast, -1)
end