...
function CreateGreevil(keys)
	...
	caster:GetOwner().greevil = CreateUnitByName("greevil_orange_player", origin, true, caster:GetOwner(), caster:GetOwner(), caster:GetTeamNumber())
	caster:GetOwner().greevil.hero = 1
	local greevil = caster:GetOwner().greevil
	greevil:SetControllableByPlayer(playerid, true)
	whistle = CreateItem("item_orange_return", greevil, greevil)
	local whistle = whistle
	whistle:SetPurchaseTime(0)
	greevil:AddItem(whistle)
	whistle = CreateItem("item_orange_whistle", caster:GetOwner(), caster:GetOwner())
	local whistle = whistle
	whistle:SetPurchaseTime(0)
